...
src_prepare() {
	# Package doesn't look for system headers
	sed -i '/pkg_check_modules(TAGLIB/ainclude_directories("/usr/include/taglib")' \
		src/CMakeLists.txt || die "sed failed for src/CMakeLists.txt"
	# wrt 709450
	sed -i  -e '/sayonara.appdata.xml/s|share/appdata|share/metainfo|' \
		-e 's|manpage/sayonara.1.gz|manpage/sayonara.1|' \
