...
src_prepare() {
	...
	sed -e s':/usr/local:/usr:' -i config/goaccess.conf || die "sed failed for goaccess.conf"

	# Respect CFLAGS
	sed -e '/CFLAGS="$CFLAGS -pthread"/'d -i configure.ac || die "sed failed for configure.ac"
