...
SRC_URI="https://ffmpeg.org/releases/${MY_P}.tar.xz
	https://dev.gentoo.org/~sam/distfiles/${CATEGORY}/${PN}/${MY_P}-texinfo.patch.xz
	verify-sig? ( https://ffmpeg.org/releases/${MY_P}.tar.xz.asc )"
...
IUSE="alsa doc +encode oss +pic sndio static-libs v4l
	${FFMPEG_FLAG_MAP[@]%:*}
	${FFMPEG_ENCODER_FLAG_MAP[@]%:*}"
...
REQUIRED_USE="cuda? ( nvenc ) glslang? ( vulkan ) libv4l? ( v4l )
	${GPL_REQUIRED_USE} ${CPU_REQUIRED_USE}"
...
PATCHES=(
	...
	"${FILESDIR}/${MY_PN}"-4.4.3-clang-14-ff_seek_frame_binary-crash.patch
	"${FILESDIR}/${MY_PN}"-4.4.3-get_cabac_inline_x86-32-bit.patch
	"${FILESDIR}/${MY_PN}"-4.4.4-wint-conversion-vulkan.patch
	"${FILESDIR}/${MY_P}"-fix-build-svt-av1-1.5.0.patch
	"${FILESDIR}/${MY_PN}"-5.1.3-binutils-2.41.patch
	"${FILESDIR}/${MY_PN}"-4.4.4-opencl-parallel-gmake-fix.patch
	"${WORKDIR}/${MY_PN}"-4.4.4-texinfo.patch
)
