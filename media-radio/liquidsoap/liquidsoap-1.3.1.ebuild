...
inherit autotools findlib python-single-r1 systemd user
...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="alsa ao camlimages +camlp4 debug dssi \
	faad fdk festival ffmpeg flac frei0r gavl gd graphics gstreamer gui \
	inotify jack json ladspa lame lastfm libsamplerate mad magic +ocamlopt ogg \
	osc opus oss portaudio profiling pulseaudio sdl shine shout soundtouch \
	speex ssl systemd taglib theora +unicode vorbis xml"
...
src_configure() {
	econf --with-user ${PN} --with-group ${PN} \
		$(use_enable camlp4) \
		...
		$(use_enable sdl) \
		$(use_enable gd) \
		$(usex ocamlopt '' --enable-custom-path) \
		--disable-optimize \
...
src_install() {
	findlib_src_install
	#todo use systemd || newinitd "${FILESDIR}"/${PN}.initd ${PN}
