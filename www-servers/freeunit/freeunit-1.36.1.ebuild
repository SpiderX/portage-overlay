...
declare -a JARS=(
	"classgraph-4.8.186:/usr/share/classgraph/lib/classgraph.jar"
	...
	"tomcat-api-9.0.121:/usr/share/tomcat-api-9/lib/tomcat-api.jar"
	"tomcat-jasper-9.0.121:/usr/share/tomcat-jasper-9/lib/tomcat-jasper.jar"
	"tomcat-jasper-el-9.0.121:/usr/share/tomcat-jasper-el-9/lib/tomcat-jasper-el.jar"
	"tomcat-jsp-api-9.0.121:/usr/share/tomcat-jsp-api-2.3/lib/tomcat-jsp-api.jar"
	"tomcat-juli-9.0.121:/usr/share/tomcat-juli-9/lib/tomcat-juli.jar"
	"tomcat-util-9.0.121:/usr/share/tomcat-util-9/lib/tomcat-util.jar"
	"tomcat-util-scan-9.0.121:/usr/share/tomcat-util-scan-9/lib/tomcat-util-scan.jar"
	"tomcat-servlet-api-9.0.121:/usr/share/tomcat-servlet-api-4.0/lib/tomcat-servlet-api.jar"
	"tomcat-el-api-9.0.121:/usr/share/tomcat-el-api-3.0/lib/tomcat-el-api.jar"
)
...
CDEPEND="${PYTHON_DEPS}"
CP_DEPEND="~dev-java/classgraph-4.8.186:0
	dev-java/eclipse-ecj:4.26
	~dev-java/jetty-http-9.4.58_p20250814:0
	~dev-java/jetty-server-9.4.58_p20250814:0
	~dev-java/jetty-util-9.4.58_p20250814:0
	~dev-java/tomcat-api-9.0.121:9
	~dev-java/tomcat-el-api-9.0.121:3.0
	~dev-java/tomcat-jasper-9.0.121:9
	~dev-java/tomcat-jasper-el-9.0.121:9
	~dev-java/tomcat-jsp-api-9.0.121:2.3
	~dev-java/tomcat-juli-9.0.121
	~dev-java/tomcat-servlet-api-9.0.121:4.0
	~dev-java/tomcat-util-9.0.121:9
	~dev-java/tomcat-util-scan-9.0.121:9"
...
BDEPEND="sys-apps/which
	...
	test? ( ${CDEPEND}
		$(python_gen_cond_dep 'dev-python/pyopenssl[${PYTHON_USEDEP}]' python3_{12..15})
		ruby? ( dev-ruby/rack
			dev-ruby/securerandom ) )"

PATCHES=( "${FILESDIR}/${PN}"-1.35.5-cflags.patch
	"${FILESDIR}/${PN}"-1.35.5-configure.patch
	"${FILESDIR}/${PN}"-1.36.1-nodejs.patch
	"${FILESDIR}/${PN}"-1.36.1-tests.patch )
...
# java-pkg-opt-2 overrides the default src_prepare,
# which is needed to apply PATCHES.
src_prepare() {
	default
}
...
src_configure() {
	...
	use nodejs && export CXXFLAGS="${CXXFLAGS} -I${S}/src -I${S}/build/include" \
