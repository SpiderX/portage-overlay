...
RDEPEND="module_config_rpcconfig? (
...
PATCHES=(
	...
	"${FILESDIR}"/patches/stg-2.408-makefile-firebird-upstream-r1.patch
	# Rewrite config for rscriptd
	"${FILESDIR}"/patches/stg-2.408-rscriptd.conf-upstream-r1.patch
	# Rewrite config for sgauth
	"${FILESDIR}"/patches/stg-2.408-sgauth.conf-upstream-r1.patch
	# Standardization of 'On-scripts'
	"${FILESDIR}"/patches/stg-2.408-on-upstream-r1.patch
	# Install demo scripts for rscriptd
	"${FILESDIR}"/patches/stg-2.408-rscriptd-upstream-r1.patch
	# Fix crush on stop
	"${FILESDIR}"/patches/stg-2.408-fix-crash-on-stop.patch
	# Rename convertor to sgconv to avoid possible file name collisions
	"${FILESDIR}"/patches/stg-2.408-sgconv-upstream-r1.patch
	...
	"${FILESDIR}"/patches/stg-2.408-build-upstream-r1.patch
	# FreeBSD install directory
	"${FILESDIR}"/patches/stg-2.408-radius-upstream-r1.patch
)
...
src_prepare() {
	...
	mv "${S}"/projects/sgconv/convertor.conf \
		"${S}"/projects/sgconv/sgconv.conf || die "Couldn't move convertor config"
	...
	sed -i 's/opts/extra_commands/' \
		"${S}"/projects/stargazer/inst/linux/etc/init.d/stargazer.gentoo \
	...
	( use module_capture_ipq && kernel_is ge 3 5 ) \
...
src_install() {
	...
	if use sgconv ; then
		cd "${S}"/projects/sgconv || die "cd to sgconv failed"
	...
	# Correct user and group for files and directories
	( use sgconv || use rscriptd || use sgauth || use stargazer ) \
